...
static void inc_starts_at(uint64_t* st_at, uint64_t bound);
static void sub_starts_at(uint64_t* st_at, uint64_t bound);
...
static inline t_win create_t_win(int h, int w, int x, int y) {
    return (t_win){
        ...
        .y = y,
        .starts_at = 0,
        .bound = 1, // bound em para nao quebrar as coisa bicho
    };
}
...
// limita ate o maximo parametriza
static void inc_starts_at(uint64_t* st_at, uint64_t bound) { 
    (*st_at)++; 
    *st_at = *st_at % bound;
}

// limita ate 0, se passar vai para o ultimo
static void sub_starts_at(uint64_t* st_at, uint64_t bound) {
    *st_at = (int64_t) (*st_at) - 1 < 0 ? bound - 1 : (*st_at) - 1;
} 

// alterna entre cada window
static void tab_win(toscop_wm* wm) {
    // fs_win é a ultima window navegavel
    wm->c_win = (wm->c_win + 1) % (FS_WIN + 1); 
}
...
void handle_key(toscop_wm *wm, int* k_p) {
    *k_p = wgetch(stdscr); // captura na win padrao 
    ...
    switch (*k_p) {
        ...
        case KEY_DOWN: {
            inc_starts_at(&get_cwin(wm)->starts_at, get_cwin(wm)->bound);
        } break;
        case KEY_UP: {
            sub_starts_at(&get_cwin(wm)->starts_at, get_cwin(wm)->bound);
        } break;
        default:
            break;
    }
...
void show_debug_info(toscop_wm* wm, double refresh_t) {
    ...
    mvwprintw(
        ...
        ",  refresh_t: %lf\n",
        fdebug, wm->c_win, 
        total_procs, get_cwin(wm)->starts_at, 
        refresh_t
    );
    refresh();
}
...
void show_toscop(toscop_wm* wm) {
    ...
    tp_print(tp, wm);             // printa o term_procs
